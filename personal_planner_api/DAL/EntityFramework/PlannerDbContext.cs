...
namespace DAL
{
    ...
    public class PlannerDbContext : IdentityDbContext<UserModel, GuidRole, Guid>
    {
        public PlannerDbContext(DbContextOptions options) : base(options)
        {
            if(Database.EnsureCreated())
            {
                var u1 = Users.Add(new UserModel { Email = "Email 1", UserName = "Username 1", PasswordHash = Hasher.HashPassword("123456789") }).Entity;
                SaveChanges();
